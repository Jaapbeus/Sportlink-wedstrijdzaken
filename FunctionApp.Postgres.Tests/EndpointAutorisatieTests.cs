...
/// <summary>
/// Autorisatie-regressietest per HTTP-endpoint (#1350) — de Postgres-tier. Tegenhanger:
/// <c>FunctionApp.Tests/Admin/EndpointAutorisatieTests.cs</c> voor de SQL Server-tier.
/// <para>
/// <b>Wat dit bewijst, voor élk HTTP-endpoint in de assembly (via reflectie, dus ook voor endpoints
/// die na deze test worden toegevoegd):</b>
/// <list type="number">
/// <item>zonder Easy Auth-principal → <c>401</c>;</item>
/// <item>met een principal die alleen de rol <c>user</c> heeft → <c>403</c>;</item>
/// <item>met de vereiste rol(len) → de poort wordt gepasseerd, en wel via <see cref="AdminEndpoint"/>.</item>
/// </list>
/// Dat laatste wordt bewezen met de testhaak <see cref="AdminEndpoint.PoortGepasseerdVoorTests"/>: die
/// geeft een <see cref="PoortGepasseerdResult"/> terug zodra de poort ná de rolcontrole is bereikt —
/// vóór de databasewacht en vóór het werk van het endpoint. Alleen de wrapper kan dat resultaat
/// opleveren, dus een endpoint dat zijn eigen poort bouwt (of geen poort heeft) valt hier door de
/// mand. Er wordt nooit een database geraakt en geen enkel endpoint doet echt werk — dat is
/// essentieel, want in de CI-job <c>fresh-db-postgres</c> staat <c>POSTGRES_CONNECTION_STRING</c>
/// gezet en zou een admin-aanroep van een DELETE- of sync-endpoint anders écht uitgevoerd worden.
/// </para>
/// <para>
/// De twee bewuste uitzonderingen (<c>AdminThemeExtract</c>, <c>AdminGeocodeGet</c>) roepen de poort
/// direct aan en lopen niet via de wrapper; voor die twee is het bewijs "mét admin géén 401/403"
/// plus de concrete <c>400</c> die hun eigen invoervalidatie geeft. <c>Health</c> is het enige
/// anonieme endpoint en wordt niet aangeroepen — een anoniem endpoint dat níet op die lijst staat,
/// faalt de <c>401</c>-test.
/// </para>
/// <para>
/// Manipuleert de procesbrede omgevingsvariabele <c>WEBSITE_SITE_NAME</c> en de statische testhaak;
/// beide worden in <c>finally</c> hersteld. De assembly draait sequentieel
/// (<c>DisableTestParallelization</c> in AssemblyInfo.cs), dus dat is veilig.
/// </para>
/// </summary>
public class EndpointAutorisatieTests
{
    ...
    /// <summary>Het enige endpoint dat bewust zonder rol bereikbaar is.</summary>
    private static readonly string[] AnoniemeRoutes = ["health"];

    /// <summary>
    /// Endpoints die de poort met opzet direct aanroepen (zie de #1350-toelichting boven elk ervan)
    /// en dus niet de testhaak maar hun eigen invoervalidatie bereiken. Waarde: de verwachte
    /// statuscode mét admin-rol op een leeg verzoek. Dezelfde twee staan, met reden, in
    /// <c>scripts/ci/endpoint-autorisatie-allowlist.txt</c>.
    /// </summary>
    ...
    /// "niets gevonden, dus alles groen"-uitkomst af als de attribuutnamen ooit wijzigen.</summary>
    ...
    public void Reflectie_VindtAlleHttpEndpoints()
    {
        ...
        endpoints.Should().Contain(e => e.Route == "postgres/sync-matches",
    ...
    {
        // #1350 (onderdeel 3): AuthorizationLevel.Admin/Function zou een Azure-key als tweede,
        // identiteitsloze toegangsweg openhouden naast Easy Auth.
        VindAlleHttpEndpoints()
            .Where(e => e.Niveau != AuthorizationLevel.Anonymous)
            .Should().BeEmpty("elk endpoint hoort op AuthorizationLevel.Anonymous + EasyAuthHelper te staan");
    }
    ...
    public async Task AdminEndpoint_ZonderPrincipal_RoeptWerkNietAan()
    {
        await MetProductieOmgeving(async () =>
        {
            ...
            var result = await AdminEndpoint.ExecuteAsync(Verzoek(null), Microsoft.Extensions.Logging.Abstractions.NullLogger.Instance, "test",
    ...
    public async Task AdminEndpoint_ZonderDatabase_ZonderPrincipal_RoeptWerkNietAan()
    {
        ...
        {
            var werkAangeroepen = false;
            var result = await AdminEndpoint.ExecuteZonderDatabaseAsync(Verzoek(null), Microsoft.Extensions.Logging.Abstractions.NullLogger.Instance, "test",
    ...
    {
        // Geen testhaak hier: dit bewijst dat de DB-loze variant het werk echt bereikt zonder
        // POSTGRES_CONNECTION_STRING of een databaseverbinding nodig te hebben.
        await MetProductieOmgeving(async () =>
        {
            var werkAangeroepen = false;
            var result = await AdminEndpoint.ExecuteZonderDatabaseAsync(
                Verzoek(Principal(("roles", "admin"))), Microsoft.Extensions.Logging.Abstractions.NullLogger.Instance, "test",
                () => { werkAangeroepen = true; return Task.FromResult<IActionResult>(new OkResult()); });
    ...
    /// <summary>Sentinel dat alleen de testhaak kan opleveren — geen enkel productiepad maakt dit type.</summary>
