...
/// <summary>
/// Postgres-tier-tegenhanger van <c>FunctionApp/Planner/Services/AvailabilityService.cs</c> (issue
/// 888 vervolg, §41). Ontsluit <c>CheckAvailability</c> en <c>DoordeweeksBeschikbaar</c>.
/// <para>
/// <b>Bewust géén live Sportlink-API-pad.</b> Het SQL Server-origineel haalt de bezetting op via
/// <c>SportlinkApiClient.GetFieldOccupationsWithApiAsync</c> — een real-time API-aanroep met een
/// DB-terugval zodra <c>useRealtimeApi = "0"</c> staat. Deze klasse gebruikt uitsluitend die
/// DB-terugval (<see cref="PlannerAvailabilityRepository.GetFieldOccupationsAsync"/>) — precies het
/// bestaande, ondersteunde configuratiepad, geen nieuw gedrag. De real-time-API-integratie zelf
/// (HTTP-aanroep + <c>EgressGuard</c>-gate + fixture-gebaseerde test, zie
/// <c>FunctionApp/Planner/SportlinkApiClient.cs</c>) is een aparte, forse eenheid werk die buiten
/// deze slice valt — expliciet genoemd, niet stilzwijgend overgeslagen.
/// </para>
/// </summary>
public static class AvailabilityService
{
    public static async Task<CheckAvailabilityResponse> CheckAvailabilityAsync(
        string connectionString, CheckAvailabilityRequest request, ILogger log, string? clubCode)
    {
        ...
        {
            var teamMatches = await PlannerMatchRepository.GetTeamMatchesOnDateAsync(connectionString, request.TeamNaam, date, cc);
        ...
        if (!string.IsNullOrEmpty(request.TeamNaam))
            teamRules = await TeamRulesRepository.GetTeamRulesAsync(connectionString, request.TeamNaam, cc);
        ...
        if (preferredTime.HasValue)
        {
            ...
            if (exactMatch != null)
            {
                ...
                AddNabijeWedstrijdWaarschuwing(response, exactMatch, duurMinuten, occupations);
        ...
        if (candidates.Count > 0)
        {
            ...
            else
            {
                ...
                AddNabijeWedstrijdWaarschuwing(response, best, duurMinuten, occupations);
    ...
    public static async Task<DoordeweeksBeschikbaarResponse> CheckDoordeweeksBeschikbaarAsync(
        string connectionString, DoordeweeksBeschikbaarRequest request, ILogger log, string? clubCode)
    {
        var cc = PostgresClubScope.Resolve(clubCode);
        ...
        {
            var speeltijd = await PlannerSettingsRepository.GetSpeeltijdAsync(connectionString, request.LeeftijdsCategorie, cc);
