...
**Empirisch geverifieerd op beide tiers** (wegwerp-Postgres-16-container en een wegwerpdatabase op
de lokale SQL-Server-2022-container): een `NULL` in de `NOT NULL`-kolom `gewijzigddoor`/
`GewijzigdDoor` forceert een echte constraintfout ná de geslaagde upsert — bewust een
databasefout en niet een kunstmatige C#-exception, zodat de test het daadwerkelijke
transactiegedrag van de engine aantoont en niet alleen de C#-controlestroom. Drie scenario's per
tier: (A) zonder transactie blijft de gewijzigde template staan zonder auditrij — de bug is dus
reproduceerbaar; (B) mét transactie is de template teruggedraaid naar zijn beginwaarde en staan er
nul auditrijen; (C) happy path commit beide rijen. Verificatie liep, net als sectie 23, via een
losse harness die de exacte transactielogica uit de fix reproduceert (geen mock van
`HttpRequest`/`FunctionContext` buiten een draaiende host — zelfde beperking als daar).
