...
| `POSTGRES_CONNECTION_STRING` | Connectiestring naar Postgres, gebruikt door `db-migrate-postgres` om de migraties vóór elke deploy toe te passen (#1093). Zelfde waarde als de Function App-instelling. Norm: `sslmode=verify-full` mét het CA-certificaat van de provider (#1004); ontbreekt dat, dan draait de app op `Require` en meldt `/api/health` een `tlsWarning` (#1095) | alleen `DatabaseTier=Postgres` |
